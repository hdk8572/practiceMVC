				...
				<ul class="nav navbar-nav navbar-right">
					...
					<c:if test="${!empty mvo}">
						<c:if test="${empty mvo.memProfile}">
							<li><img class="img-circle" src="${contextPath}/resources/images/person.png" style="widows: 50px; height: 50px"> ${mvo.memName} 님
							(
						...
						<c:if test="${!empty mvo.memProfile}">
							<li><img class="img-circle" src="${contextPath}/resources/upload/${mvo.memProfile}" style="widows: 50px; height: 50px"> ${mvo.memName} 님
							(
								<c:forEach var="authVo" items="${mvo.authList}">
									<c:if test="${authVo.auth eq 'ROLE_USER'}">U</c:if>
									<c:if test="${authVo.auth eq 'ROLE_MANAGER'}">M</c:if>
